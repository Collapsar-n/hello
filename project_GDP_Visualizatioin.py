...
def read_csv_as_nested_dict(filename, keyfield, separator, quote):  # 读取原始csv文件的数据，格式为嵌套字典
    ...
    """
    result = {}
    with open("isp_gdp.csv", newline="")as csvfile:
...
def reconcile_countries_by_name(plot_countries, gdp_countries):  # 返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    for j in plot_countries:
        country_name = plot_countries[j]
        result = country_name in gdp_countries
        if result == True:
            dict1[j] = country_name
        if result == False:
            set1.add(j)
    tuple1 = (dict1, set1)
    return tuple1
...
def test_render_world_map(year):  # 测试函数
    ...
    render_world_map(gdpinfo, pygal_countries, year, "qqisp_gdp_world_name_1966.svg")
